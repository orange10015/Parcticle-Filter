...
def mouseCallback(event, x, y, flags, null):             #x,y是图像坐标系坐标
    global center
    global trajectory
    global previous_x
    global previous_y
    ...
    center = np.array([[x, y]])
    trajectory = np.vstack((trajectory, np.array([x, y])))  #垂直方向堆叠数组形成新数组
    ...
    c[0]=sum(particles[:,0])/400
    c[1]=sum(particles[:,1])/400
    previous_x = x
    previous_y = y
...
def update(particles, weights, z, R, landmarks):
    ...
    for i, landmark in enumerate(landmarks):
        j=0
        distance = np.power((particles[:, 0] - landmark[0]) ** 2 + (particles[:, 1] - landmark[1]) ** 2, 0.5)#计算每个粒子到地标i的距离
...
def systematic_resample(weights):
    N = len(weights)
    positions = (np.arange(N) + np.random.random()) / N         #np.arange(N)生成从0到N的固定步长的排列

    indexes = np.zeros(N, 'i')
    cumulative_sum = np.cumsum(weights)         #当前列之前的和加到当前列上
    i, j = 0, 0
    while i < N and j < N:
...
def resample_from_index(particles, weights, indexes):         #根据指定的样本进行重采样
...
while (1):
    ...
    if cv2.waitKey(DELAY_MSEC) & 0xFF == 27:   #如果按下ESC退出循环
        break
